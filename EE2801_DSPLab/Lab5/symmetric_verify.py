...
from symmetric import pyhw_symmetric
...
for i in range(1, 4):
    ...
    verilog_raw = read_hex_file(os.path.join('data', f'out_symmetric_sig{i}.hex'))
    
    sig_padded = sig_raw + [0] * (num_taps - 1)
    pyhw_out = pyhw_symmetric.hw_fir_symmetric(sig_padded, coeffs_raw)
    ...
    fig.suptitle(f'Signal {i} Analysis for symmetric')
    ...
    plt.savefig(f"plots/symmetric_sig{i}.png")
    plt.show()
