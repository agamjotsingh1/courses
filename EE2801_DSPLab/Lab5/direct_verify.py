...
from direct import pyhw_direct
...
for i in range(1, 4):
    ...
    verilog_raw = read_hex_file(os.path.join('data', f'out_direct_sig{i}.hex'))
    
    sig_padded = sig_raw + [0] * (num_taps - 1)
    pyhw_out = pyhw_direct.hw_fir_direct(sig_padded, coeffs_raw)
    ...
    fig.suptitle(f'Signal {i} Analysis for Direct')
    ...
    plt.savefig(f"plots/direct_sig{i}.png")
    plt.show()
