module fir_symmetric #(
    ...
    wire [DATA_WIDTH-1:0] symmetric_sum;
    reg [DATA_WIDTH-1:0] symmetric_sum_reg;
    ...
    fp_mul #(
        ...
        .num1(symmetric_sum_reg),
        .num2(coeff_in[count_prod*DATA_WIDTH +: DATA_WIDTH]),
        .out(prod)
    );
        ...
        if(rst) begin
            ...
            case(state)
                ...
                DOING: begin
                    count <= count + 1;
                    count_prod <= count;
                    symmetric_sum_reg <= symmetric_sum;
                    prod_reg <= prod;
                    accum <= next_accum;
                    if(count == (NUM_TAPS/2 + 1)) state <= DONE;
                end
                DONE: begin
                    if(next == 1'b0) begin
                        state <= DONE;
